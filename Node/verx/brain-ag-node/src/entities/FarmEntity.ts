...
export class FarmEntity {
    ...
    isValid():{valid: boolean, errors: string[]} {
        ...
        // type check
        if (typeof this.totalArea != 'number') errors.push("Wrong type of totalArea.");
        if (typeof this.arableArea != 'number') errors.push("Wrong type of arableArea");
        if (typeof this.vegetationArea != 'number') errors.push("Wrong type of vegetationArea");

        // check total area
        if(this.totalArea && this.arableArea && this.vegetationArea){
            const isAreaValid = (this.arableArea + this.vegetationArea) <= this.totalArea
            if(!isAreaValid) errors.push("arableArea plus vegetationArea needs to be less or equal total area");
        }
